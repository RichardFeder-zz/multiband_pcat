...
def make_fourier_templates(N, M, n_terms, extradimfac=1.0, show_templates=False):
    ...
    templates = np.zeros((n_terms, n_terms, N, M))
    ...
    xtemps = np.zeros((n_terms, N, M))
    ytemps = np.zeros((n_terms, N, M))
    for n in range(n_terms):
        xtemps[n] = np.sin((n+1)*np.pi*meshx/(N*extradimfac))
        ytemps[n] = np.sin((n+1)*np.pi*meshy/(M*extradimfac))
    
    counter=1
    for i in range(n_terms):
        for j in range(n_terms):
            templates[i,j,:,:] = xtemps[i]*ytemps[j]
...
def generate_template(fourier_coeffs, n_terms, fourier_templates=None, N=None, M=None):
    ...
    sum_temp = np.sum([fourier_coeffs[i,j]*fourier_templates[i,j] for i in range(n_terms) for j in range(n_terms)], axis=0)
...
def fit_coeffs_to_observed_comb(observed_comb, obs_noise_sig,ftemplates, true_fcoeffs = None, true_comb=None, n_terms=None, sig_dtemp=0.1, niter=100, init_nsig=1.):
    if true_fcoeffs is not None:
        init_fcoeffs = np.random.normal(0, obs_noise_sig, size=(true_fcoeffs.shape[0], true_fcoeffs.shape[1]))
        n_terms = init_fcoeffs.shape[0]

    elif n_terms is not None:
        init_fcoeffs = np.random.normal(0, obs_noise_sig, size=(n_terms, n_terms))
            
    running_fcoeffs = init_fcoeffs.copy()
    
    all_running_fcoeffs = np.zeros((niter//1000, n_terms, n_terms))
    ...
    for n in range(niter):
        ...
        prop_dtemp = ftemplates[np.random.randint(0, n_terms),np.random.randint(0, n_terms),:,:]*perts[n]
        ...
        if accept_or_not:
            ...
            running_fcoeffs[idx0, idx1] += perts[n]
            lnLs[n] = plogL
            lnL = plogL
        else:
            lnLs[n] = lnL
    ...
    print('mean acceptance fraction is ', np.mean(accepts))
