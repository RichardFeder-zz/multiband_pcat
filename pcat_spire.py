...
def get_parser_arguments():
	...
	parser.add_argument('--nregions', type=int, default=5, help='splits up image into subregions to do proposals within')
	parser.add_argument('--psf_pixel_fwhm', type=int, default=3)
	parser.add_argument('--nominal_nsrc', type=int, default=300, help='this is the nominal number of sources expected in a given image, helps set \
		sample step sizes during MCMC')
	...
	parser.add_argument('--self.nloop', type=int, default=1000, help='factor by which the chain is thinned')
...
def get_spire_psf(pixel_fwhm=3, nc=5):
    
    gaussian_2D_kernel = Gaussian2DKernel(pixel_fwhm)
    persf = np.zeros((50,50))
    persf[0:25,0:25] = gaussian_2D_kernel.array
...
def load_in_map(opt, band=0, zero_nans=True):
    ...
    if zero_nans:
        ...
        exposure[np.isnan(exposure)] = 0.0
...
class Model:
    ...
    def __init__(self, opt):
        ...
        self.n = np.random.randint(opt.nstar)+1
		...
		self.stars = np.zeros((2+opt.nbands,opt.nstar), dtype=np.float32)
        self.stars[:,0:self.n] = np.random.uniform(size=(2+opt.nbands,self.n))  # refactor into some sort of prior function?
    ...
    def print_sample_status(self, dts, accept, outbounds, chi2):    
    ...
    def run_sampler(self):
        ...
        rtype_array = np.random.choice(self.moveweights.size, p=moveweights, size=self.nloop)
        ...
        timestat_array, accept_fracs = self.print_sample_status(dts, accept, outbounds, chi2)
    ...
    def move_stars(self): 
        ...
        dpos_rms = np.float32(np.sqrt(N_eff/(2*np.pi))*err_f/(np.sqrt(self.nominal_nsrc*self.regions_factor*(2+self.nbands))))/(np.maximum(f0[0],pfs[0]))
...
opt.regsize = opt.imsz[0]/opt.nregions
opt.regions_factor = 1./float(opt.nregions**2)
...
xsample = np.zeros((opt.nsamp, opt.nstar), dtype=np.float32)
ysample = np.zeros((opt.nsamp, opt.nstar), dtype=np.float32)
timestats = np.zeros((opt.nsamp, 6, 5), dtype=np.float32)
...
accept_stats = np.zeros((opt.nsamp, 5), dtype=np.float32)
tq_times = np.zeros(opt.nsamp, dtype=np.float32)
if multiband:
    fsample = [np.zeros((opt.nsamp, opt.nstar), dtype=np.float32) for x in xrange(nbands)]
    colorsample = [[] for x in xrange(opt.nbands-1)]
else:
    fsample = np.zeros((opt.nsamp, opt.nstar), dtype=np.float32)
chi2sample = np.zeros((opt.nsamp, opt.nbands), dtype=np.int32)
model = Model()
#create directory for results

frame_dir, newdir = create_directories(opt.timestr)
...
for j in xrange(nsamp):
    ...
    _, chi2_all[k], statarrays,  accept_fracs, diff2_list, rtype_array, accepts = models[k].run_sampler()
